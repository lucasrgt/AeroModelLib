...
final class Aero_AnimationBudgetPressure {
    ...
    static void updateVisibleChunks(int chunks) {
        ...
        int target = chunks >= Aero_AnimationBudgetConfig.VISIBLE_CHUNK_HIGH
            ? Math.min(maximum, Aero_AnimationBudgetConfig.VISIBLE_CHUNK_HIGH_MAX)
            : chunks >= Aero_AnimationBudgetConfig.VISIBLE_CHUNK_MID
                ? Math.min(maximum, Aero_AnimationBudgetConfig.VISIBLE_CHUNK_MID_MAX) : maximum;
        ...
        Aero_AnimationBudgetEngine.visibleLimit = current;
    }
    static void record(double frameMs, double displayMs, double chunkMs, long gcMs) {
        if (!enabled() || Aero_AnimationBudgetEngine.accepted <= 0 && Aero_AnimationBudgetEngine.rejected <= 0) return;
        ...
        boolean driver = displayMs >= Aero_AnimationBudgetConfig.DISPLAY_STALL_MS
            || chunkMs >= Aero_AnimationBudgetConfig.RENDER_CHUNK_STALL_MS;
        boolean pressure = driver || gcMs >= Aero_AnimationBudgetConfig.GC_STALL_MS
